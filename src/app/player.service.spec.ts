...
describe('PlayerService', () => {
  ...
  const httpClientSpy: jasmine.SpyObj<HttpClient> = jasmine.createSpyObj('httpClient', ['get', 'post', 'put']);
  ...
  it('should add a player', () => {
    ...
    service.addPlayer(players[0]).subscribe(result => {
      expect(httpClientSpy.post).toHaveBeenCalledWith(service.playersUrl, players[0], service.httpOptions);
  ...
  it('should get a player', () => {
    ...
    service.getPlayer(players[1].name).subscribe(result => {
      expect(httpClientSpy.get).toHaveBeenCalledWith(service.playersUrl + "/" + players[1].name);
  ...
  it('should update a player', () => {
    ...
    service.updatePlayer(players[0]).subscribe(result => {
      expect(httpClientSpy.put).toHaveBeenCalledWith(service.playersUrl, players[0], service.httpOptions);
      expect(result).toEqual(players[0]);
    });
  })
