...
describe('GameItemComponent', () => {
  ...
  it('should display a gory losing downward facing played game item', (done) => {
    ...
    fixture.whenStable().then(() => {
      imgElement = fixture.nativeElement.querySelector('img');
      expect(imgElement).toBeTruthy();
      expect(imgElement.src.endsWith(`assets/images/${component.itemType}-vs-${component.opponentItemType}.png`)).toBeTrue()
      expect(imgElement.getAttribute("data-played")).toEqual(`${component.played}`);
      expect(imgElement.getAttribute("data-orientation")).toEqual(`${component.orientation}`);
...
@Component({
  template: '<app-game-item [itemType]="itemType" [played]="played"  [orientation]="orientation" [opponentItemType]="opponentItemType" [goreEnabled]="goreEnabled" (onClick)="onClick($event)"></app-game-item>'
})
